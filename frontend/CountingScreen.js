...
const CountingScreen = () => {
  ...
  const AnimatedObject = ({ type, emoji, size }) => {
    ...
    else if (["twinkle","grow"].includes(type)) style = { transform: [{ scale: anim.interpolate({ inputRange:[0,1], outputRange:[1,1.2] }) }] };
  ...
  const getObjectSize = (count, type) => {
    let size;
    if(count <= 2) size = 60;
    else if(count <= 4) size = 50;
    else if(count <= 6) size = 40;
    else if(count <= 8) size = 32;
    else size = 28;

    if(type === "twinkle" || type === "grow") size -= 15;
    return size;
  };
