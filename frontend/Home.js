...
const handleLogout = async () => {
  try {
    ...
    // 1. Notify backend (optional but good for screenTime ending)
    if (token) {
      ...
      <Modal visible={drawerVisible} animationType="slide" transparent>
        <View style={styles.fullDrawerWrapper}>
          <View style={styles.drawerContent}>
            {[{tab: 'Home', icon: 'home'}, {tab: 'Profile', icon: 'user'}, {tab: 'Settings', icon: 'settings'}, {tab: 'Notifications', icon: 'bell'}, {tab: 'AboutUs', icon: 'info'}].map(({tab, icon}) => (
              <TouchableOpacity key={tab} style={styles.drawerItem} onPress={() => { setSelectedTab(tab); setDrawerVisible(false); }}>
...
const styles = StyleSheet.create({
  ...
  clearBtn: { alignSelf: 'flex-end', padding: 8, backgroundColor: '#EF3349', borderRadius: 6, marginBottom: 10 },
  clearBtnText: { color: '#fff' },

  notificationBox: { padding: 14, marginBottom: 10, borderRadius: 10 },
  unread: { borderLeftWidth: 4, borderLeftColor: 'green' },
  read: { borderLeftWidth: 4, borderLeftColor: '#ccc' },
  notificationMessage: { color: '#444', fontSize: 14 },
...
noteCardNew: {
  width: '100%',
  backgroundColor: '#A0F0DC', // solid mint background
  borderRadius: 10,
  paddingVertical: 16,
  paddingLeft: 20,
  paddingRight: 45,
...
noteStripeNew: {
  ...
  left: 0,
  width: 4,
  backgroundColor: '#EF3349', // PrepPal red stripe
...
unread: {
  borderLeftWidth: 4,
  borderLeftColor: '#2BCB9A', // Mint for unread
},
read: {
  borderLeftWidth: 4,
  borderLeftColor: '#ccc',
